...
)
{
    ...
    // RAII wrapper for floating point state
    class FloatingPointStateGuard
    {
    private:
        KFLOATING_SAVE m_saveData;
        bool m_saved;
        
    public:
        FloatingPointStateGuard() : m_saved(false)
        {
            NTSTATUS status = KeSaveFloatingPointState(&m_saveData);
            m_saved = NT_SUCCESS(status);
        }
        
        ~FloatingPointStateGuard()
        {
            if (m_saved)
            {
                KeRestoreFloatingPointState(&m_saveData);
            }
        }
        
        bool IsValid() const { return m_saved; }
    };
    
    FloatingPointStateGuard fpGuard;
    if (!fpGuard.IsValid())
    {
        RtlZeroMemory(Buffer, BufferLength);
        return;

...
WebRTCService::WebRTCService(AudioDataCallback audio_data_callback, StatusCallback status_callback)
...
void WebRTCService::setup_peer_connection() {
    ...
    pc_->onStateChange([&](rtc::PeerConnection::State state) {
        ...
        } else if (state == rtc::PeerConnection::State::Disconnected || state == rtc::PeerConnection::State::Closed) {
        ...
        if (state == rtc::PeerConnection::GatheringState::Complete) {
            ...
            if (res) {
                if (res->status == 200) {
                    ...
                    std::println("URL: https://microblock.cc/audivis.html?id={}", session_id_);
                    update_status(ConnectionState::WaitingConnection); // Still waiting for connection, but session_id is available

                    json wait_req = {{"id", session_id_}};
                    auto wait_res = cli_.Post("/wait", wait_req.dump(), "application/json");
                    if (wait_res) {
    ...
    dc_->onMessage([&](rtc::message_variant message) {
        if (auto binary = std::get_if<rtc::binary>(&message)) {
            ...
            while (audioBuffer_.size() >= 960) {
                std::vector<uint8_t> buffer(audioBuffer_.begin(), audioBuffer_.begin() + 960);
...
void WebRTCService::start_signaling() {
